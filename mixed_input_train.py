...
def train_model_grid(rnn_type, class_weights, param_grid, metrics, save_file_suffix, verbose):
    ...
    for params in list(param_grid):
        ...
        if rnn_type == 'GRU':
            model = create_gru_model(rnn_input_shape=rnn_input_shape, dnn_input_shape=dnn_input_shape, output_shape=output_shape,
                           rnn_dim=rnn_dim, final_dense_dim=final_dense_dim, dropout=dropout,
                           regularization=reg_l2, mask_value=mask_val)
        elif rnn_type == 'LSTM':
            model = create_lstm_model(rnn_input_shape=rnn_input_shape, dnn_input_shape=dnn_input_shape, output_shape=output_shape,
                           rnn_dim=rnn_dim, final_dense_dim=final_dense_dim, dropout=dropout,
                           regularization=reg_l2, mask_value=mask_val)
...
train_model_grid(rnn_type='GRU', class_weights=class_weights,
    param_grid=param_grid_fl, metrics=metrics, save_file_suffix='-gru-fl', verbose=verbose)

# Train LSTM models with focal loss
train_model_grid(rnn_type='LSTM', class_weights=class_weights,
